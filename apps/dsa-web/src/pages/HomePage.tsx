...
// History list state
  ...
  useTaskStream({
    ...
    onTaskFailed: (task) => {
      updateTask(task);
      // Show error prompt
      setStoreError(getParsedApiError(task.error || '分析失敗'));
        ...
        <div className="flex items-center gap-2 w-full min-w-0 flex-1" style={{ maxWidth: 'min(100%, 1168px)' }}>
          ...
          <div className="flex-1 relative min-w-0">
            <input
              ...
              placeholder="輸入股票程式碼，如 600519、00700、AAPL"
      ...
      <section className="md:col-start-4 md:row-start-2 flex-1 overflow-y-auto overflow-x-auto px-3 py-4 md:px-0 md:py-5 md:pl-2 md:pr-1 min-w-0 min-h-0">
        ...
        {isLoadingReport ? (
          <div className="flex flex-col items-center justify-center h-full">
            ...
            <p className="mt-3 text-secondary-text text-sm">載入報告中...</p>
          </div>
        ) : selectedReport ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-center">
            ...
            <h3 className="text-base font-medium text-foreground mb-1.5">開始分析</h3>
            <p className="text-xs text-muted-text max-w-xs">
              輸入股票程式碼進行分析，或從左側選擇歷史報告檢視
            </p>
          </div>
        )}
      </section>
